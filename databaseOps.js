...
module.exports = {
  ...
  // get_current_userId: get_current_userId  
}
...
const checkProfileDB = "SELECT count(GoogleID) from ProfileTable WHERE GoogleID = (?)";
const insertProfileDB = "insert into ProfileTable (GoogleID, firstName) values (?,?)";
// const allProfileDB = "SELECT * from ProfileTable";
const getGoogleID = "SELECT GoogleID from ProfileTable WHERE GoogleID = ?";
...
// sameerah 4/9 
// function to insert profile in db if not already there
// async function insert_profile(id, name) {
//   // check how many of profile is in DB
//   try {
//     let numprofile = await db.get(checkProfileDB, [id]);
//     // if no occurrences, enter into database
//     console.log("numprofile: ", numprofile);
//     if (numprofile.length == 0) {
//       try {
//         await db.run(insertProfileDB, [id, name]);
//         console.log("Inserted a new User with GoogleID: ", id);
//       }
//       catch (error) {
//         console.log(error);
//       }
//     } else{
//       console.log("This is the current user GoogleID: ", id);
//     }
//   }
//   catch (error) {
//     console.log(error);
...
async function insert_profile(id, name) {
  ...
  try {
    ...
    // if no occurrences, enter into database
    console.log("numprofile: ", numprofile);
    console.log("numprofile['count(GoogleID)']: ", numprofile['count(GoogleID)']);
    if (numprofile['count(GoogleID)'] == 0) {
...
async function get_all_profile() {
  try {
    ...
    // for easy test to look for data from console
    console.log("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
    console.log("all profile: ", results);
    return results;
  } 
  catch (error) {
    console.log("get_all_profile error");
    console.log(error);
    return [];
  }
}
...
//   try{
//     let current_userId = await db.get("SELECT GoogleID from ProfileTable WHERE GoogleID = ?", []);
//     console.log("The id: ", current_userId);
...
async function get_most_recent_planned_activity_in_range(min, max) {
  try {
    console.log("reminder currentUserId", currentUserId);
    let results = await db.get(getMostRecentPrevPlannedDB, [currentUserId, min, max]);
...
async function get_most_recent_entry() {
  try {
    let result = await db.get(getMostRecentDB, [currentUserId]);
...
async function get_similar_activities_in_range(activityType, min, max) {
  try {
    let results = await db.all(getPastWeekByActivityDB, [currentUserId, activityType, min, max]);
    return results;
  }
  catch (error) {
    console.log(error);
...
async function delete_past_activities_in_range(min, max) {
  try {
    await db.run(deletePrevPlannedDB, [currentUserId, min, max]);
  }
  catch (error) {
    console.log(error);
  }
