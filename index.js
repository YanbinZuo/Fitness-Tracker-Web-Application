...
// db is never used, so we can comment out
...
// Added ID and Key to Secret tab, replaced code here -PS

const hiddenClientID = process.env['googleClientID']
const hiddenClientSecret = process.env['googleClientKey']
//Added userProfile
let userProfile;
...
// there is a code: app.get('/*', express.static('public'));
...
app.get('/auth/accepted',
  ...
  //EDIT 5/7: change to user profile
  //EDIT:5/9 changed userProfile to name.givenName
  //NOTE: Test with both Patrick's emails to see if same data is loaded based on user name
  function(req, res) {

    console.log("Inside auth profile has arrived: ", userProfile);
    console.log('Logged in and using cookies!')
    // tell browser to get the hidden main page of the app

    console.log("Request Body: " + JSON.stringify(req.body));
    res.redirect(`/public/index.html?userName=${userProfile.name.givenName}`);
...
// EDIT 5/7: add from TA example
// seems just for testing, so we can comment out
...
// someone in discord said this is just an example
...
// EDIT 5/8: merge from fit2 solution for all queries function
// add isAuthenticated callback for each function
...
app.get('/reminder', isAuthenticated, async function(request, response, next) {
  ...
  // check to see all profile database
  dbo.get_all_profile().catch(
    function(error) {
...
function gotProfile(accessToken, refreshToken, profile, done) {
  ...
  dbo.insert_profile(userid, userFirstName).catch(
    function(error) {
      console.log("error:", error);
    }
  );
...
// EDIT 5/8: merge helper function from fit2 solution
